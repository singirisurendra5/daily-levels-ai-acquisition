...
CREATE TABLE IF NOT EXISTS signals (
 signal_id TEXT PRIMARY KEY, platform TEXT, url TEXT, text TEXT, date TEXT, source TEXT, ingested_at TEXT,
 ...
 def upsert_signals(self,df):
  ...
  with self._conn() as con:
   for _,r in df.iterrows():
    ...
    vals[cols.index('qualification_version')] = safe_text(r.get('qualification_version','3.6-final')) or '3.6-final'
    vals[cols.index('unmet_need')] = int(bool(r.get('unmet_need',False)))
    vals[cols.index('sales_ready')] = int(bool(r.get('sales_ready',False)))
    vals[cols.index('lead_reason')] = safe_text(r.get('lead_reason',''))
