...
## V3.5.1 Lead Qualification Engine
V3.5.1 separates **Relevance**, **Buying Intent**, and **Product Fit**. Priority is a composite qualification score, so generic trading discussions, moderator posts, personal recaps, and existing alternative level sources do not automatically become HOT leads.

Qualification fields: `relevance_score`, `buying_intent_score`, `product_fit_score`, `priority_score`, `competition_detected`. Use **Re-qualify stored signals** after upgrading an existing deployment so older records receive the V3.5.1 scoring model.

Target funnel: Raw signals → Relevant → High-intent → High-fit → true HOT opportunities.


## V3.5.1 — Qualification Consistency Fix
